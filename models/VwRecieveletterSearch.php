...
class VwRecieveletterSearch extends VwRecieveletter
{
    ...
    public function search($params)
    {
        ...
        if ($UserID) {
            $query = VwRecieveletter::find()->where(['UsersID_Reciever' => $UserID])->andWhere(['LettersDraftType' => 2]);
        } else {
            throw new NotFoundHttpException('خطا در vwRecieveLettersSarch');
        }
