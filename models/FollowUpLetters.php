...
/**
 * FollowUpLetters represents the model behind the search form of `app\models\VwRecieveletter`.
 */
class FollowUpLetters extends VwRecieveletter
{
    /**
     * {@inheritdoc}
     */
    ...
    public function search($params)
    {
        ...
        if ($UserID) {
            $query = VwRecieveletter::find()->select([
                'LettersID',
                'LettersSubject',
                'LettersAbstract',
                'LettersNumber',
                ...
                'PersianLettersDraftType',
                // 'PersianSendLettersReadType',
            ])->where(['UsersID_FK' => $UserID])->andWhere(['LettersDraftType' => 2])->andWhere(['LettersFollowType' => 1]);
        } else {
            throw new NotFoundHttpException('خطا در FollowUpLetters');
        }
