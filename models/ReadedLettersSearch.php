...
/**
 * ReadedLettersSearch represents the model behind the search form of `app\models\VwRecieveletter`.
 */
class ReadedLettersSearch extends VwRecieveletter
{
    ...
    public function search($params)
    {
        ...
        if ($UserID) {
            $query = VwRecieveletter::find()->where(['UsersID_Reciever' => $UserID])->andWhere(['LettersDraftType' => 2])->andWhere(['SendLettersReadType' => 2]);
        } else {
            throw new NotFoundHttpException('خطا در ReadedLettersSearch');
        }
