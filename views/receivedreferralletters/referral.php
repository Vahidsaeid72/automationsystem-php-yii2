...
$('#frm-referral').on('beforeSubmit', function(e){
 ...
 $.ajax({
    ...
    success: function(data){
        history.pushState(null,'','/automationsystem/receivedreferralletters');
        $.pjax.reload({container:"#vw-referralletters-index",async:false});

        $('#cl_close_receivedReferral').click();
        NProgress.done();
    }
...
<div class="alert alert-info text-center">
    <h5 class="text-primary">مشاهده اطلاعات نامه با شماره : <strong><?= $LetterNumber ?></strong></h5>
    <h5 class="text-primary">ایجادکننده نامه : <strong><?= $creator ?></strong></h5>
    <h5 class="text-primary">ارجاع دهنده : <strong><?= $referraler ?></strong></h5>
